...
vector<string>		list_file_delete;
string 				*text_list_delete[30];
...
static void	modify_index(t_data data)
{
	...
	if (*index + delta >= -27 && *index + delta < (int)(((list_file_delete.size() / 3) * 3 + 3)))
		*index += delta;
	for (int i = 0; i < 30; i++)
	{
		if (*index + i >= (int)(list_file_delete.size()) || *index + i < 0)
			*(text_list_delete[i]) = "";
		else
			*(text_list_delete[i]) = list_file_delete[i + *index];
	}
}
...
void			menu_delete_actor(t_data data) // 0 - t_gui * / 1 - t_actor * / 2 - &name / 3 - &path / 4 - &pool
{
	...
	while (i < 30)
	{
		t_button *button = new t_button(new t_text_button(
					(i < (int)(list_file_delete.size()) ? list_file_delete[i] : ""), BLACK,
		...
		text_list_delete[i] = &(button->button->text);
		gui.add(button);
		i++;
	}
